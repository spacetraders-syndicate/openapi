...
describe('system locations', () => {
  ...
  beforeAll(async () => {
    ...
    const { data: { systems: returnedSystems } } = await api.listGameSystems();
  ...
  it(
    ...
    async () => {
      const { data: { locations: locationsAllowingConstruction } } = await api.listSystemLocations({
        symbol: systems[0].symbol,
      ...
      const { data: { locations: locationsTypeMoon } } = await api.listSystemLocations({
        symbol: systems[0].symbol,
        type: "MOON"
      })
  ...
  it(
    ...
    async () => {
      const { data: { location } } = await api.getGameLocation({
  ...
  it(
    ...
    async () => {
        ...
        const { data: { location } } = await api.listGameLocationsShips({
